<%@ Page Title="PLMS - Customer lookup" Language="C#" AutoEventWireup="true" CodeFile="cluAgentLM.aspx.cs" Inherits="_cluAgentLM" %>
...
<head runat="server">
<title>Search for Left Messages by Agent</title>
...
<tr>
     ...
     <td>
          <obout:OboutDropDownList ID="txtapptset" runat="server" ToolTip="Select an appointment setter if applicable">
                <asp:ListItem>None</asp:ListItem>
                <asp:ListItem>Mary MsDonough</asp:ListItem>
...
<asp:HyperLink runat="server" ID="HyperLink1" Text="Return to Partner Pages" NavigateUrl="~/default.aspx"></asp:HyperLink>
</div>
</asp:Panel>
<asp:SqlDataSource ID="sds1" runat="server" SelectCommand="SELECT * FROM [StatusDDL] ORDER BY [Status] DESC" ConnectionString="<%$ ConnectionStrings:salespipeline %>" />
		    
<asp:SqlDataSource ID="SqlDataSource1" runat="server" 
    ConnectionString="<%$ ConnectionStrings:salespipeline %>" 
    OldValuesParameterFormatString="original_{0}" 
    SelectCommand="SELECT * FROM [Customers] WHERE ([Agent] = @Agent) AND [Status] = 'Left Message' ORDER BY [LDate] DESC">
    <SelectParameters>
