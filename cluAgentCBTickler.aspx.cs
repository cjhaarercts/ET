...
public partial class _cluAgentCBTickler : Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            // keep calendar min/current behavior
            Calendar1.DateMin = DateTime.Now;
            GridView1.DataBind();
        }

        // Emit a safe client-side variable for JavaScript to use instead of inline <%=%> code blocks.
        // Register once per page request if control exists.
    ...
    protected void ImageButton1_Click(object sender, ImageClickEventArgs e)
    {
        ...
        // Safely apply branch value to dropdown-like control (txtbranch)
        string branchValue = gvRow.Cells[13].Text == null ? string.Empty : gvRow.Cells[13].Text.Trim();
        try
        {
            if (txtbranch != null)
            {
                // supports both standard DropDownList and controls exposing Items
                ListItem found = null;
                if (txtbranch.Items != null)
                {
                    found = txtbranch.Items.FindByText(branchValue) ?? txtbranch.Items.FindByValue(branchValue);
                }
                ...
                else
                {
                    ...
                    if (txtbranch.Items != null)
                    {
                        txtbranch.Items.Insert(0, new ListItem(branchValue, branchValue));
                        txtbranch.ClearSelection();
                        txtbranch.Items[0].Selected = true;
                    }
                    else
                    {
                        // fallback: set Text when Items not available
                        txtbranch.Text = branchValue;
                    }
                }
            }
            else
        ...
        // Safely apply honorable discharge value
        string dischargeValue = gvRow.Cells[14].Text == null ? string.Empty : gvRow.Cells[14].Text.Trim();
        try
        {
            if (txthdischarge != null)
            {
                ListItem foundDis = null;
                if (txthdischarge.Items != null)
                {
                    foundDis = txthdischarge.Items.FindByText(dischargeValue) ?? txthdischarge.Items.FindByValue(dischargeValue);
                }

                if (foundDis != null)
                {
                    txthdischarge.ClearSelection();
                    foundDis.Selected = true;
                }
                else
                {
                    if (txthdischarge.Items != null)
                    {
                        txthdischarge.Items.Insert(0, new ListItem(dischargeValue, dischargeValue));
                        txthdischarge.ClearSelection();
                        txthdischarge.Items[0].Selected = true;
                    }
                    else
                    {
                        txthdischarge.Text = dischargeValue;
                    }
                }
            }
            else
        ...
        txtDate.Text = gvRow.Cells[17].Text.Trim() == "&nbsp;" ? "" : gvRow.Cells[17].Text;
    ...
    private void UpdateCustomerRecord(DateTime? appointmentDate)
    {
                                         ...
                                         WHERE Id=@Id", con))
        {
            ...
            // typed parameter for appointment date
    ...
    private DateTime? ParseDate(string input)
    {
        ...
        // explicit out variable for older/legacy compilers
        DateTime parsed;
    ...
    {
        // classic switch to support older compilers
        switch (agent)
        {
